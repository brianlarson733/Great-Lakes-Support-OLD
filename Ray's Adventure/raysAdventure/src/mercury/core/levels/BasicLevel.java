...
public abstract class BasicLevel {
	...
	public static void makeDecision(Ray ray) {		
		while(!endLevel){
			...
			// determine the list of choices to present to the player
			if(ray.getLocation().getBeings().size() > 1 ){
				String[] choices = {"Inspect the room", "Inspect the items you are carrying", "Go to another room","Interact with another beings in the room"};	
			}
			else {
